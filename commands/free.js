    ...
    async execute(interaction) {
        ...
        if (interaction.channelId !== config.genChannel) {
            const wrongChannelEmbed = new EmbedBuilder()
                ...
                .setDescription(`You cannot use the \`/free\` command here. Please use it in <#${config.genChannel}>!`)
        ...
        if (!isAdmin && generated.has(member.id)) {
            const cooldownEmbed = new EmbedBuilder()
                .setColor(config.color.red)
                .setTitle('Cooldown!')
                .setDescription(`Please wait **${config.genCooldown}** seconds before using the command again.`)
                .setFooter({ text: interaction.user.tag, iconURL: interaction.user.displayAvatarURL({ dynamic: true, size: 64 }) })
                .setTimestamp();
            if (replied) {
        ...
        // Check if service exists in the directory
